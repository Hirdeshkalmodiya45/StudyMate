  ...
  const fetchQuestions = async (topic, category) => {
    ...
    try {
        ...
        rawData = results.map((q) => {
          ...
          return {
            ...
            answer: q.correct_answer
              ...
              .replace(/&#039;/g, "'")
              .replace(/&amp;/g, "&"),
            explanation: "Explanation not available from source.",
            topic,
          };
        });
      } else {
      ...
      const normalized = Array.isArray(rawData)
        ? rawData
        : rawData.questions || [rawData];

      if (!Array.isArray(normalized) || normalized.length === 0) {
        throw new Error("No questions returned from API");
      }

      const filtered = normalized.filter((q) => q.topic === topic || !q.topic);
      return filtered.length ? filtered : normalized;
    } catch (err) {
      const errors = {
        Aptitude: "❌ Failed to fetch Aptitude questions",
        Reasoning: "❌ Failed to fetch Reasoning questions",
        "Verbal Ability": "❌ Failed to fetch Verbal Ability questions",
      };
      throw new Error(errors[category] || "❌ Failed to fetch questions");
    }
  };
